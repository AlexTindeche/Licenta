...
def determine_last_track_id(detection, track_history, frame, frame_nr, frame_history, frame_eps = 120*30, eps = 100, siam_thresh = 0.5):
...
class Run(QObject):
    ...
    def __init__(self, filename, output, zones, ways):
        super().__init__()
    ...
    def angular_similarity(self, traj1, traj2):
        ...
        # Plot the vectors
        # plt.quiver(traj1[0][0], traj1[0][1], vector1[0], vector1[1], angles='xy', scale_units='xy', scale=1)
        # plt.quiver(traj1[-1][0], traj1[-1][1], vector2[0], vector2[1], angles='xy', scale_units='xy', scale=1)
        # plt.plot(traj1[:, 0], traj1[:, 1])
        # plt.plot(traj2[:, 0], traj2[:, 1])
        # # Plot the starting and ending points
        # plt.scatter(traj1[0][0], traj1[0][1], c='red')
        # plt.scatter(traj1[-1][0], traj1[-1][1], c='red')
        # plt.scatter(point1[0], point1[1], c='green')
        # plt.scatter(point2[0], point2[1], c='green')
        # plt.show()
    ...
    def predict(self):
        ...
        with VideoSink(self.TARGET_VIDEO_PATH, video_info) as sink:
            ...
            for frame_nr, frame in enumerate(tqdm(generator, total=10000), start=1):
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.lineLayoutWays = QHBoxLayout()
        # Add a textbox to the tab 2 for introducing the number of zones
        self.outputFileLabel4 = QLineEdit(self.tab2)
        self.outputFileLabel4.setText('Numarul de sensuri de mers >')
        self.outputFileLabel4.setReadOnly(True)
        self.outputFileLabel4.setFixedWidth(self.outputFileLabel.fontMetrics().boundingRect(self.outputFileLabel4.text()).width() + 10)
        self.outputFileLabel4.setAlignment(Qt.AlignLeft)
        self.lineLayoutWays.addWidget(self.outputFileLabel4)

        self.outputFileLineEdit4 = QLineEdit(self.tab2)
        self.outputFileLineEdit4.setPlaceholderText('Numarul de sensuri de mers (DOAR INTEGERS)')
        self.lineLayoutWays.addWidget(self.outputFileLineEdit4)
        self.outputFileLineEdit4.editingFinished.connect(lambda: self.captureNumberWays(self.outputFileLineEdit4.text()))

        self.tab2Layout.addLayout(self.lineLayoutWays)
    ...
    def captureNumberWays(self, text):
        try:
            self.ways = int(text)
        except ValueError:
            print("Please introduce an integer value")
    ...
    def run_script(self):
        ...
        if self.filename is None or self.output is None or self.zones is None or self.ways is None:
            print("Va rugam sa completati toate campurile")
            QMessageBox.warning(self, 'EROARE', 'Va rugam sa completati toate campurile')
            return
        else:
            reply = QMessageBox.question(self, 'Message', 'Sunteti sigur ca datele introduse sunt corecte? Numel fisierului de input: ' + self.filename + ' Numel fisierului de output: ' + self.output + ' Numarul de zone: ' + str(self.zones) + ' Numarul de sensuri de mers: ' + str(self.ways), QMessageBox.Yes | QMessageBox.No, QMessageBox.No)
            ...
            if reply == QMessageBox.Yes:
                ...
                self.runWorker = Run(self.filename, self.output, self.zones, self.ways)
    ...
    def on_run_finished(self):
        ...
        self.outputFileLineEdit3.clear()
        self.outputFileLineEdit4.clear()
